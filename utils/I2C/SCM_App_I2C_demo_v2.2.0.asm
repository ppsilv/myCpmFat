; **********************************************************************
; **  I2C demo       v2.2.0  29-Aug-2024        by Stephen C Cousins  **
; **********************************************************************
...
;                                       7-bit     Address byte
; Default I2C addresses      Device     Address   Write   Read     Alternatives
; =====================      ======     =======   =====   ====     ============
; SC138 Memory card          24LC256    0x57      0xAE    0xAF     None
; SC401 Prototyping module   ?          ?         ?       ?        ?
; SC402 Memory module        24xxx      0x50      0xA0    0xA1     1010 AAA-
; SC403 Digital I/O module   MCP2318    0x20      0x40    0x41     0100 AAA-
; SC404 Digital I/O module   MCP2308    0x21      0x42    0x43     0100 AAA-
; SC405 Digital I/O module   PCF8574/A  0x3A      0x74    0x75     0100 AAA- / 0111 AAA-
; SC406 Temperature sensor   TC74 A5    0x4D      0x9A    0x9B     Alternative chips
; SC407 Switches and lights  PCF8574/A  0x27/3F   0x4E/7E 0x4F/7F  0100 AAA- / 0111 AAA-
;
; Notes: 
...
; HISTORY
; v2.0.0   10-Feb-2021   Version 2 released
; v2.1.0   26-Feb-2021   More tests added
; v2.2.0   29-Aug-2024   Added display of I2C bus port address
;                        Changed to 70bit address notation
;                        Added test for I2C bus master present
...
; The default device option is currently for SC126 or compatibles 
;#DEFINE    SC_DEFAULT          ;Default I2C bus master port
;#DEFINE    SC126@0C            ;SC126 I2C module at I/O address 0x0C
#DEFINE    SC137@20             ;SC137 I2C module at I/O address 0x20

; I2C device 7-bit addresses
SC138_7BIT  .EQU 0x57           ;SC402 Memory card (24xxx)
SC401_7BIT  .EQU 0x00           ;SC401 User defined
SC402_7BIT  .EQU 0x50           ;SC402 Memory card (24xxx)
SC403_7BIT  .EQU 0x20           ;SC403 Digital I/O module (MCP23018)
SC404_7BIT  .EQU 0x21           ;SC404 Digital I/O module (MCP23008)
SC405_7BIT  .EQU 0x3A           ;SC405 Digital I/O module (PCF8574A)
SC406_7BIT  .EQU 0x4D           ;SC406 Temperature sensor (TC74)
SC407_7BIT  .EQU 0x3F           ;SC407 Switches and lights (PCF8574A)

; I2C device write addresses used by this demonstration and test program:
SC138_ADDR  .EQU SC138_7BIT * 2 ;SC402 Memory card (24xxx)
SC401_ADDR  .EQU SC401_7BIT * 2 ;SC401 User defined
SC402_ADDR  .EQU SC402_7BIT * 2 ;SC402 Memory card (24xxx)
SC403_ADDR  .EQU SC403_7BIT * 2 ;SC403 Digital I/O module (MCP23018)
SC404_ADDR  .EQU SC404_7BIT * 2 ;SC404 Digital I/O module (MCP23008)
SC405_ADDR  .EQU SC405_7BIT * 2 ;SC405 Digital I/O module (PCF8574A)
SC406_ADDR  .EQU SC406_7BIT * 2 ;SC406 Temperature sensor (TC74)
SC407_ADDR  .EQU SC407_7BIT * 2 ;SC407 Switches and lights (PCF8574A)
...
; Main program
            ...
            LD   DE,@MsgVers    ;Address of version message string
            CALL StrOut         ;Output string

            LD   DE,@MsgPort    ;Address of port address string
            CALL StrOut         ;Output string
            LD   A,I2C_PORT     ;Get address of device tested
            CALL HexOut         ;Output as two character hex 
            LD   DE,@MsgHex     ;Address of 'values in hex' string
            CALL StrOut         ;Output string

            CALL I2C_TEST       ;Test is bus master present
            JR   NZ,@Abort      ;Not found so abort
            LD   DE,@MsgFound   ;Address of 'bus master detected' string
            CALL StrOut         ;Output string
...
; Bus master not found
@Abort:     LD   DE,@MsgAbort   ;Address of 'bus master not found' string
            CALL StrOut         ;Output string
            RET

@MsgVers:   .DB  "I2C demo/test v2.2.0 by Stephen C Cousins",0x0D,0x0A,0
@MsgPort:   .DB  "I2C bus master I/O port address: ",0
@MsgHex:    .DB  " (all values in hex)",0x0D,0x0A,0
@MsgFound:  .DB  "I2C bus master detected",0x0D,0x0A,0
@MsgAbort:  .DB  "I2C bus master not found",0x0D,0x0A,0
...
@LOOP:      PUSH DE             ;Preserve DE
            ...
            SRL  A              ;Convert to 7-bit i2c device address
            CALL HexOut         ;Output as two character hex 
...
@Msg:       .DB  "I2C devices found at (7-bit hex addresses): ",0
...
; Display device report, eg.
;   SC407 Switches and Lights Module at Address=<XX> Write=<XX> Read=<XX+1>
;         <demo/test> result
;   On entry: A = Address of device on I2C bus (write address)
;             DE = Address of null terminated string
...
Report:     LD   H,A            ;Store device write address
            ...
            SRL  A              ;Convert to 7-bit device address
            LD   B,A            ;Store 7-bit device address
            ...
            CALL LineOut        ;Output new line
            RET
@Address:   .DB  " at Address=$B (Write=$H Read=$L)",0
...
; **********************************************************************
; I2C bus master test (by reading back SDA line)
;   On entry: A = 0 and Z flagged if bus master detectedd
;   On exit:  BC DE HL IX IY preserved

I2C_TEST:   CALL I2C_SDA_LO     ;SDA low
            CALL I2C_RdPort     ;Read SDA input
            BIT  I2C_SDA_RD,A   ;Test SDA input
            JR   NZ,@Failed     ;Skip if SDA not low
            CALL I2C_SDA_HI     ;SDA high
            CALL I2C_RdPort     ;Read SDA input
            BIT  I2C_SDA_RD,A   ;Test SDA input
            JR   Z,@Failed      ;Skip if SDA not high
            XOR  A              ;I2C bus master detected
            RET                 ;  so return with A=0 and Z flagged

@Failed:    OR   0xFF           ;Failed to find bus master
            RET                 ;  so return with A!=0 and NZ flagged
